...
PsActivity.prototype.action_remove_link_preview = function (post_id, act_id) {
	...
	if ($act.length) {
		$act.find('.ps-js-post-body .js-stream-attachments').empty();
	...
	if ($ai.length > 0) {
		...
		peepso.postJson('activity.savecomment', req, function (json) {
			if (json.success) {
				...
				jQuery(
					'[data-comment-id=' + post_id + '] .js-stream-content > .js-stream-attachments'
...
PsActivity.prototype.option_edit = function (post_id, act_id) {
	...
	var $ct = jQuery('.ps-js-activity--' + act_id),
		...
		$attachments = $ct.find('.ps-js-post-body .cstream-attachments'),
