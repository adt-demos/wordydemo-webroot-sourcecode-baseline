		...
		if ($match) {
			...
			foreach ($this->user_ids as $user_id) {
				...
				if($user_id != $post_act->act_owner_id) {
					...
					$notifications->add_notification(intval($post->post_author), $user_id, $_notification, 'tag', self::MODULE_ID, $post_id, $act_id);
				}
		        else
		        {
		...
		if ($match) {
			...
			foreach ($this->user_ids as $user_id) {
		        ...
		        if ((!in_array($user_id, $follower) && ($user_id != $post_act->act_owner_id)) || ($post_act->act_owner_id == $user_id && intval($post_act->act_comment_object_id) > 0)) {
					...
					$notifications->add_notification(intval($post->post_author), $user_id, $_notification, 'tag_comment', self::MODULE_ID, $post_id, $act_id);
		        }
		        else
		        {
		        	// if tagged, modify notification
