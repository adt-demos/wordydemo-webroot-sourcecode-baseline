...
//			'post_date_gmt' => date('Y-m-d H:i:s'), // gmdate('Y-m-d H:i:s'),
        ...
        if ($author !== $owner) {
            ...
            $note->add_notification($author, $owner, __('wrote on your profile', 'peepso-core'), 'wall_post', self::MODULE_ID, $id, $act_id);
        }
        ...
        if (NULL !== $repost) {
            ...
            $note->add_notification($owner, $orig_post->post_author, __('shared', 'peepso-core'), 'share', self::MODULE_ID, $id, $act_id);
        }
...
//			PeepSoMailQueue::add($owner_id, $data, __('Someone Commented on Your Post', 'peepso-core'), 'usercomment');
			...
			if ($orig_post->post_type == PeepSoActivityStream::CPT_POST) {
				...
				$note->add_notification($author_id, $owner_id, __('commented', 'peepso-core'), 'user_comment', self::MODULE_ID, $id, $act_id);
			} else {
				...
				$note->add_notification($author_id, $owner_id, __('replied to', 'peepso-core'), 'stream_reply_comment', self::MODULE_ID, $id, $act_id);
			}
        ...
        while ($users->have_posts()) {
            ...
            $note->add_notification($author_id, $users->post->post_author, __('commented', 'peepso-core'), 'user_comment', self::MODULE_ID, $id, $act_id);
        }
...
//				PeepSoMailQueue::add($owner_id, $data, __('Someone liked your post', 'peepso-core'), 'likepost');
                ...
                $note->add_notification($user_id, $owner_id, $notification_message, 'like_post', $module_id, $post_id, $act_id);
            }
        } else {
    ...
    public function savepost(PeepSoAjaxResponse $resp)
    {
        ...
        if (empty($post_content) && !apply_filters('peepso_activity_allow_empty_content', FALSE)) {
            ...
            // notify post owner
            if ($owner_id !== $user_id) {
                $note->add_notification($user_id, $owner_id, __('updated a post', 'peepso-core'), 'wall_post', self::MODULE_ID, $post_id, $act_id);
            }
            ...
            while ($users->have_posts()) {
                ...
                if (intval($users->post->post_author) !== $owner_id && intval($users->post->post_author)  !== $user_id) {
                    $note->add_notification($user_id, $users->post->post_author, __('updated a post', 'peepso-core'), 'wall_post', self::MODULE_ID, $post_id, $act_id);
                }
            }
